...
export default function DocumentationView({ 
  ...
  bodyRawDoc, authDoc, updateHeader, updatePathParam, updateBodyParam, updateRequestInCollection, bodyParams, addHeader, addPathParam, removeHeader, removePathParam,
  ...
  const requestList = requests.length > 0 
    ? requests.map(r => r.id === activeRequestId ? { ...r, ...requestProp } : r)
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 space-y-8">
      ...
      {requestList.map((request, reqIdx) => {
        // No modo editor, só mostramos os campos se for a request ativa
        const isTargetOfEdit = !isEditing || (activeRequestId === request.id);
        ...
        const activeBodyParams = (request.id === activeRequestId) ? bodyParams : (request.bodyParams || []);

        return (
          <div key={request.id} className={`${!isEditing && reqIdx > 0 ? 'pt-12 border-t border-slate-200 dark:border-slate-800' : ''}`}>
            {!isEditing && (
              <div className="flex items-center gap-3 mb-8">
                <span className={`text-[10px] font-black px-2 py-1 rounded-lg border ${methodStyles[request.method]}`}>{request.method}</span>
                <h2 className="text-xl font-black text-slate-800 dark:text-white">{request.name}</h2>
                <p className="text-[10px] font-mono text-slate-400 truncate flex-1">{request.url}</p>
              </div>
            )}
            ...
            {isEditing && (
              <div className="grid grid-cols-1 gap-4 mb-8">
                ...
                <div className="flex gap-2">
                  <select className={`input-base !w-32 font-bold border ${methodStyles[request.method]}`} value={request.method} onChange={(e) => setMethod(e.target.value)}>
                    {['GET', 'POST', 'PUT', 'DELETE', 'PATCH'].map(m => <option key={m} value={m}>{m}</option>)}
                  </select>
                  <input className="input-base font-mono" value={request.url} onChange={(e) => setUrl(e.target.value)} />
                </div>
              </div>
            )}
      ...
      <div className="space-y-12">
        {/* Descrição */}
        <section>
          <SectionHeader title="Documentação Geral" icon={<svg className="w-4 h-4 text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>} />
          {isEditing ? (
            <textarea 
          ...
          {authExpanded && (
            <div className="p-6 bg-emerald-500/5 border border-emerald-500/10 rounded-2xl">
              <div className="flex items-center gap-2 text-sm font-bold dark:text-slate-200">
                ...
                {isEditing ? (
                  <select className="input-base !py-1 !text-xs bg-transparent max-w-xs" value={request.authType} onChange={(e) => setAuthType(e.target.value)}>
                  ...
                  request.authType === 'none' ? "Sem Autenticação" : request.authType.toUpperCase()
                )}
              </div>
              {isEditing ? (
        ...
        {request.bodyType !== 'none' && (
          <section>
            <SectionHeader 
              title={`Request Body (${request.bodyType})`} 
            ...
            {['json', 'xml', 'text'].includes(request.bodyType) && (
              <div className="mb-4">
                {isEditing ? (
                  <textarea 
            ...
            {isEditing && ['json', 'xml', 'text'].includes(request.bodyType) && (
               <div className="mb-6">
            ...
            {!isEditing && ['json', 'xml', 'text'].includes(request.bodyType) && (
              ...
              <pre className="bg-slate-950 p-6 rounded-2xl text-slate-300 font-mono text-xs overflow-x-auto border border-slate-800">
                {request.bodyRaw || "// Sem corpo definido"}
              </pre>
              <button 
                onClick={() => copyToClipboard(request.bodyRaw)}
