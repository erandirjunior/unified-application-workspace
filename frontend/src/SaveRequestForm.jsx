...
export default function SaveRequestForm({ onSaveRequest, requestName, setRequestName }) {
  const handleSave = () => {
  ...
  return (
    <div className="flex flex-col sm:flex-row gap-4 items-end bg-slate-50 dark:bg-slate-800/50 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm mb-6">
      <div className="flex-1 w-full">
        <label htmlFor="request-name" className="label-base">Nome da Requisição</label>
        <input id="request-name" type="text" value={requestName} onChange={(e) => setRequestName(e.target.value)} className="input-base" placeholder="Ex: Criar Novo Usuário" />
      </div>
