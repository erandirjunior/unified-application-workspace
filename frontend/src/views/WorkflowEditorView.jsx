...
export default function WorkflowEditorView({ workflow, onUpdateWorkflow, onBack, onRun, onEditStep, collection, t }) {
  ...
  // Fábricas de steps
  const makeRequest = () => ({ id: Date.now().toString() + Math.random().toString(36).substr(2, 9), type: 'request', name: 'HTTP Request', method: 'GET', url: 'https://api.example.com', totalRequests: 1, duration: 0, headers: [], bodyType: 'none' });
        ...
        <div className="lg:col-span-1 space-y-4">
          ...
          <div className="p-4 theme-surface rounded-2xl border theme-border">
            ...
            <div className="flex flex-col gap-1.5">
              {[
                { fn: () => addStep(makeRequest()), icon: '⚡', label: t.workflows?.editor?.addAction || 'HTTP Request', color: 'blue' },
                { fn: () => setIsCopyModalOpen(true), icon: '📋', label: t.workflows?.editor?.copyLabel || 'Copiar HTTP', color: 'emerald' },
                ...
                <button key={i} onClick={item.fn} className={`flex items-center gap-3 p-2.5 w-full theme-elevated border theme-border rounded-xl hover:border-${item.color}-500/30 transition-all text-left`}>
